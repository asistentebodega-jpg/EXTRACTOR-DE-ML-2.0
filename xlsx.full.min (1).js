        ...
        const App = {
            ...
            downloadsWatcher: {
                ...
                setButtonState(state = 'idle', detail = '') {
                    ...
                    button.title = detail || (
                        active
                            ? 'Detener vigilancia de Descargas'
                            : 'Vigilar Descargas y cargar automaticamente el Excel nuevo'
                    );
                ...
                async chooseDirectory() {
                    if (!App.downloadsWatcher.isSupported()) {
                        throw new Error('Tu navegador no permite vigilar carpetas desde esta pagina. Usa Chrome o Edge actualizado y selecciona la carpeta Descargas cuando se solicite.');
                    }
                ...
                async start() {
                    ...
                    if (newestInitialFile) {
                        App.runtime.downloadsWatch.pendingFiles.set(newestInitialFile.key, {
                            size: newestInitialFile.file.size,
                            lastSeenAt: Date.now()
                        });
                    }
                    ...
                    App.ui.showMessage(
                        'Vigilando Descargas. Se cargaran automaticamente los archivos GridViewSolicitudDespacho nuevos.',
                        'success',
                        { title: 'Descargas' }
                    );
